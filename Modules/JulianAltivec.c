...
//  const vector float VFZero = VF_ALL (0.0);
//  const vector float VMaxDist = VF_ALL (4.0);
//  const vector float VEpsilon = VF_ALL (1e-5); // arbitrary epsilon precision check set to 1e-5 due single precision floats

  const vector float VJKre = VF_ALL (JKre);
  const vector float VCre = (vector float) { Cre, Cre1, Cre2, Cre3 };
  const vector float VFZero = vec_xor (VFZero,VFZero);
  const vector float VJKim = VF_ALL (JKim);
